...
const InvitationStatusModal = ({ isOpen, onClose, projectId, onRefresh }: InvitationStatusModalProps) => {
  ...
  const fetchInvitations = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/projects/${projectId}/invitations`, {
        headers: {
  ...
  const handleResendInvitation = async (invitationId: string, email: string) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:3000/api/invitations/${invitationId}/resend`, {
        method: "POST",
        headers: {
