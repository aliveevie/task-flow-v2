...
const AdminProjects = () => {
  ...
  const handleEditProject = async (data: any) => {
    try {
      const response = await fetch(`http://api.galaxyitt.com.ng:3000/api/projects/${editingProject.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleDeleteProject = async (projectId: string) => {
    try {
      const response = await fetch(`http://api.galaxyitt.com.ng:3000/api/projects/${projectId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
